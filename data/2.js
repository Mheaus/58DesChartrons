...
const flatImages = importAll(require.context('../../assets/image/58-des-chartrons/', false, /\.(png|jpe?g|svg)$/));
...
class FlatPage extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      background: ""
  ...
  render() {
    return(
      <Layout className="flat" background={this.state.background}>
        ...
        <RoomList
          rooms={flat.rooms}
          onImageClick={(image) => this.changeBackground(image)}
        />
      </Layout>
    )
  }
